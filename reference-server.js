...
/**
 * POST /api/marketing
 * body: { customers: Array<{ id, name, email?, preferences?: string[], history?: Array<{product:string, amount:number, date:string, category?:string}> }> }
 * returns: { emails: Array<{ customerId, subject, preview, body, segments: string[] }> }
 */
app.post('/api/marketing', async (req, res) => {
  try {
    ...
    // If a BQ model is available, attempt structured generation
    if (BQ_MODEL) {
      try {
        const payload = customers.slice(0, 20).map(c => ({
          id: c.id,
          name: c.name,
          preferences: (c.preferences || []).join(', '),
          history: (c.history || []).slice(-5)
        }));
        const prompt = `Create highly personalized marketing emails for each of these customers using their purchase history and preferences. Return structured JSON rows with fields: customerId, subject, preview, body, segments[]. Customers: ${JSON.stringify(payload)}`;
        const sql = `
          SELECT *
          FROM AI.GENERATE_TABLE(
            MODEL \`${BQ_MODEL}\`,
            (
              SELECT @prompt AS prompt,
              STRUCT<customerId STRING, subject STRING, preview STRING, body STRING, segments ARRAY<STRING>> AS response_schema
            )
          )`;
        const rows = await runQuery(sql, { prompt });
        if (rows && rows.length) {
          return res.json({ emails: rows.map(r => ({
        ...
        console.warn('marketing AI.GENERATE_TABLE failed, falling back:', err?.message || err);
      }
    }

    // Fallback deterministic templating
    const emails = customers.map(c => {
      const last = (c.history || []).slice(-1)[0] || { product: 'our collection', category: 'featured' };
      const prefs = (c.preferences || []).slice(0, 3).join(', ');
      const segments = [last.category || 'general'].concat((c.preferences || []).slice(0, 2));
      const subject = `${c.name ? c.name + ',' : 'You,'} a special pick inspired by your recent ${last.product}`;
      const preview = `Handpicked for you${prefs ? ` — based on ${prefs}` : ''}`;
      const body = `Hi ${c.name || 'there'},\n\nWe noticed your interest in ${last.product}. Based on your preferences${prefs ? ` for ${prefs}` : ''}, we curated a short list we think you'll love.\n\n• Exclusive offer on complementary items\n• Fresh arrivals in ${last.category || 'our catalog'}\n• Quick reorder options\n\nAs a thank-you, enjoy 10% off with code PERSONAL10 (48h).\n\nBest,\nYour Climate–Economy Insight Assistant`;
      return { customerId: c.id, subject, preview, body, segments };
    });
    return res.json({ emails });
  } catch (err) {
    console.error('marketing error', err);
    return res.status(500).json({ error: 'marketing failed', message: err?.message || String(err) });
  }
});

/**
 * POST /api/support
 * body: { logs: string | string[] }
 * returns: { summary: string, categories: Array<{name, count}>, topIssues: string[], recommendations: string[], sentiment: 'positive'|'neutral'|'negative' }
 */
app.post('/api/support', async (req, res) => {
  try {
    const logs = Array.isArray(req.body?.logs) ? req.body.logs : [String(req.body?.logs || '')];
    const text = logs.filter(Boolean).join('\n');
    if (!text.trim()) return res.status(400).json({ error: 'logs required' });

    if (BQ_MODEL) {
      try {
        const prompt = `Summarize and categorize these support call/chat logs. Return JSON fields: summary STRING, categories ARRAY<STRUCT<name STRING, count INT64>>, topIssues ARRAY<STRING>, recommendations ARRAY<STRING>, sentiment STRING. Logs: ${text.substring(0, 10000)}`;
        const sql = `
          SELECT * FROM AI.GENERATE_TABLE(
            MODEL \`${BQ_MODEL}\`,
            (
              SELECT @prompt AS prompt,
              STRUCT<summary STRING, categories ARRAY<STRUCT<name STRING, count INT64>>, topIssues ARRAY<STRING>, recommendations ARRAY<STRING>, sentiment STRING> AS response_schema
            )
          )`;
        const rows = await runQuery(sql, { prompt });
        if (rows && rows[0]) {
          ...
          return res.json({
            ...
            sentiment: String(row.sentiment || 'neutral').toLowerCase()
          });
        }
      } catch (err) {
        console.warn('support AI.GENERATE_TABLE failed, fallbacking:', err?.message || err);
      }
    }
    ...
    const keywords = [
      { name: 'Billing', match: /(refund|invoice|charge|billing)/g },
      { name: 'Login', match: /(login|password|reset|2fa)/g },
      { name: 'Performance', match: /(slow|latency|timeout)/g },
      { name: 'Bug', match: /(error|bug|crash|issue)/g },
    ];
    const categories = keywords.map(k => ({ name: k.name, count: (lower.match(k.match) || []).length }))
      .filter(k => k.count > 0)
      .sort((a, b) => b.count - a.count);
    const sentiment = lower.includes('angry') || lower.includes('frustrat') || lower.includes('refund') ? 'negative' : lower.includes('thanks') ? 'positive' : 'neutral';
    const topIssues = categories.slice(0, 5).map(c => c.name);
    const recommendations = [
      'Create macros for top recurrent issues',
      'Add in-product tips for frequent confusion points',
      'Prioritize fixes for top categories in next sprint'
    ];
    const summary = `Processed ${logs.length} logs. Top categories: ${topIssues.join(', ') || 'None'}. Overall sentiment: ${sentiment}.`;
    return res.json({ summary, categories, topIssues, recommendations, sentiment });
  } catch (err) {
    console.error('support error', err);
    return res.status(500).json({ error: 'support failed', message: err?.message || String(err) });
  }
});